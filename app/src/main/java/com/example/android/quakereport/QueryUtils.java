...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONObject jsonObjEarthquakeData = new JSONObject(SAMPLE_JSON_RESPONSE);

            //Extract “features” JSONArray
            JSONArray JsonArrEarthquakes = jsonObjEarthquakeData.getJSONArray("features");

            //Loop through each feature in the array
            for (int i = 0; i < JsonArrEarthquakes.length(); i++) {
                //Get earthquake JSONObject at position i
                JSONObject myJsonObject = JsonArrEarthquakes.getJSONObject(i);
                //Get “properties” JSONObject
                JSONObject myJsonObjectProperties = myJsonObject.getJSONObject("properties");
                // Extract “mag” for magnitude
                mag = myJsonObjectProperties.getDouble("mag");
                // Extract “place” for location
                place = myJsonObjectProperties.getString("place");
                // Extract “time” for time
                time = myJsonObjectProperties.getLong("time");
                // Extract URL
                url = myJsonObjectProperties.getString("url");

                // Create Earthquake java object from magnitude, location, and time
                Earthquake earthquake = new Earthquake(place, time, mag, url);
